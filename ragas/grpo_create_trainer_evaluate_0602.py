...
class RAGOptimizer:
    ...
    def __init__(self, 
                 ...
                 project_dir: str,
                 config_path: str = "/home/cz/AUTO_GRADON/ragas/configuration/config212new.yaml",
    ...
    def generate_config(self, actions: np.ndarray) -> str:
        ...
        Args:
            actions: 动作数组，形状为 (num_components,)，每个值在 [0, 7] 范围内
            
        Returns:
            生成的配置文件路径，如果失败返回 None
        """
        try:
            self.logger.info(f"[DEBUG generate_config] 开始生成配置，输入动作: {actions}")
            
            # 🔧 新增：映射动作到组件配置
            mapped_actions = self._map_actions_to_components(actions)
            if mapped_actions is None:
                self.logger.error("[DEBUG generate_config] 动作映射失败")
                return None
            
            self.logger.info(f"[DEBUG generate_config] 映射后的动作: {mapped_actions}")
            
            # 将映射后的动作转换为选择的方法
            selected_methods = {}
            components_to_process = [comp for comp in self.components if comp in self.base_config]
            
            for i, component in enumerate(components_to_process):
                # 🔧 使用映射后的动作索引
                if i >= len(mapped_actions):
                    self.logger.error(f"[DEBUG generate_config] 映射动作索引 {i} 超出范围，mapped_actions长度: {len(mapped_actions)}")
                    return None
                
                action_idx = mapped_actions[i]
                self.logger.info(f"[DEBUG generate_config] 组件 {component} 的映射动作索引: {action_idx}")
            ...
            self.logger.error(f"[DEBUG generate_config] 生成配置过程出错: {str(e)}")
            return None
    ...
    def _evaluate_batch(self, actions_batch, gpu_id: int) -> List[float]:
        ...
        for i, actions in enumerate(actions_batch):
            ...
            try:
                ...
                # 存储详细信息以便后续显示
                action_key = str(tuple(map(int, actions)))
                self.batch_details[action_key] = {
                    'rouge': rouge_score,
                    'meteor': meteor_score,
    ...
    def evaluate_configs(self, actions_batch) -> List[float]:
        ...
        for i, action_tuple in enumerate(actions_tuples):
            # 🔧 修改：先映射动作，再获取策略
            mapped_actions = self._map_actions_to_components(list(action_tuple))
            if mapped_actions is None:
                self.logger.error(f"动作映射失败: {action_tuple}")
                mapped_actions = [0] * len(self.components)
            
            # 获取当前动作对应的策略
            selected_methods = {}
            for j, mapped_component_idx in enumerate(mapped_actions):  # 🔧 使用映射后的动作
                if j < len(self.components):
                    component = self.components[j]
                    node_key = f"node{j+1}"
                    action_idx = str(mapped_component_idx)  # 🔧 使用映射后的索引
    ...
    def _summarize_batch_results(self, actions_batch, rewards):
        ...
        for i, actions in enumerate(actions_batch):
            ...
            # 获取策略
            selected_methods = {}
            for j, mapped_action in enumerate(mapped_actions):
                if j < len(self.components):
                    component = self.components[j]
                    node_key = f"node{j+1}"
                    action_idx = str(int(mapped_action))
                    if action_idx in self.nodes_config[node_key]:
                        selected_methods[component] = self.nodes_config[node_key][action_idx]
    ...
    def generate_actions_with_elite(self, num_samples, epsilon):
        ...
        # 🔧 修改：确保动作在0-7范围内（不需要裁剪到实际操作数）
        # 因为映射函数会处理这个问题
        actions_np = np.clip(actions_np, 0, 7)
    ...
    def run_optimization(self, num_epochs: int = 20, batch_size: int = 20):
        ...
        for epoch in range(num_epochs):
            ...
            max_reward_actions = tuple(map(int, actions_np[max_reward_idx]))
            max_reward_key = str(max_reward_actions)
            
            if hasattr(self, 'batch_details') and max_reward_key in self.batch_details:
                details = self.batch_details[max_reward_key]
                self.logger.info(f"  - 最大奖励: {metrics['max_reward']:.4f} (ROUGE={details['rouge']:.4f}, METEOR={details['meteor']:.4f}, BERT={details['bert_score']:.4f})")
            else:
    ...
    def _map_actions_to_components(self, actions):
        """
        将动作映射到组件配置
        actions: 动作向量 (7维，每维0-7)
        return: 映射后的组件动作 (7维，每维对应实际组件选项数)
        """
        # 🔧 添加调试信息
        ...
        try:
            ...
            if len(actions) >= 6:
                ...
                self.logger.error(f"[DEBUG] 动作长度不足: {len(actions)}, 期望6")
                return None
            
            mapped_actions = []
            
            # 定义映射规则
            # 节点1,2,4,5,7: 4个选项 (0,1→0, 2,3→1, 4,5→2, 6,7→3)
            four_option_mapping = {0: 0, 1: 0, 2: 1, 3: 1, 4: 2, 5: 2, 6: 3, 7: 3}
            
            # 节点3: 8个选项 (直接映射)
            eight_option_mapping = {i: i for i in range(8)}
            
            # 节点6: 2个选项 (0,1,2,3→0, 4,5,6,7→1)
            two_option_mapping = {0: 0, 1: 0, 2: 0, 3: 0, 4: 1, 5: 1, 6: 1, 7: 1}
            
            # 🔧 新增：prompt_maker的3个选项映射 (0,1→0, 2,3,4→1, 5,6,7→2)
            three_option_mapping = {0: 0, 1: 0, 2: 1, 3: 1, 4: 1, 5: 2, 6: 2, 7: 2}
            
            for i, action in enumerate(actions):
                if i == 0:  # retrieval (4个选项)
                    mapped_actions.append(four_option_mapping[action])
                elif i == 1:  # query_expansion (4个选项)
                    mapped_actions.append(four_option_mapping[action])
                elif i == 2:  # passage_reranker (8个选项)
                    mapped_actions.append(eight_option_mapping[action])
                elif i == 3:  # passage_filter (4个选项)
                    mapped_actions.append(four_option_mapping[action])
                elif i == 4:  # passage_compressor (4个选项)
                    mapped_actions.append(four_option_mapping[action])
                elif i == 5:  # passage_augmenter (2个选项)
                    mapped_actions.append(two_option_mapping[action])
                elif i == 6:  # prompt_maker (3个选项)
                    mapped_actions.append(three_option_mapping[action])
...
if __name__ == "__main__":
    # 配置路径
    #qa_data_path = "../data/rag-mini-bioasq/sampled20_qa_formatted.parquet"
    #corpus_data_path = "../data/rag-mini-bioasq/sampled20_corpus_formatted.parquet"
    qa_data_path = "../data/5dataset_100/sampled20_qa.parquet"
    corpus_data_path = "../data/5dataset_100/sampled20_corpus.parquet"
    #project_dir = "../experiments/214-re-qe-par8-paf-pac-pau-pm"
    project_dir = "../experiments/212-qa20"
    ...
    optimizer = RAGOptimizer(
        ...
        num_gpus=3,
        target_components=["retrieval", "query_expansion", "passage_reranker", "passage_filter", "passage_compressor", "passage_augmenter"],
        fixed_components={
