...
from grpo import GRPOTrainer, extract_diverse_qa_features, debug_feature_diversity
...
class RAGOptimizer:
    ...
    def setup_grpo_trainer(self, d_model: int = 256, nhead: int = 8, num_layers: int = 3):
        ...
        # 🚀 使用新的多样化特征提取
        self.logger.info(f"🔍 从问答数据中提取多样化特征: {self.qa_data_path}")
        qa_features = extract_diverse_qa_features(self.qa_data_path, len(self.components))
        self.logger.info(f"✅ 多样化特征提取完成，形状: {qa_features.shape}")
        
        # 调试特征多样性
        debug_feature_diversity(qa_features)
        ...
        self.trainer = GRPOTrainer(
            ...
            kl_coeff=0.01,
            clip_eps=0.2,
            #qa_features=qa_features  # 使用多样化特征
        )
        
        self.node_operations = operations
        self.logger.info(f"GRPO训练器初始化完成，每个进程都有独特的输入特征")
        return self.trainer
    ...
    def _evaluate_batch(self, actions_batch, gpu_id: int) -> List[Dict]:
        """
        在特定GPU上评估一批配置
    ...
    def evaluate_configs(self, actions_batch) -> List[float]:
        ...
        # 初始化存储详细信息的属性
        if not hasattr(self, 'current_batch_details'):
            self.current_batch_details = {}
        ...
        for i, action_tuple in enumerate(actions_tuples):
            ...
            if cache_key in self.evaluation_cache:
                ...
                if isinstance(cached_data, dict):
                    ...
                    self.current_batch_details[cache_key] = cached_data
                else:
                    # 兼容旧的缓存格式（只有奖励值）
                    reward = cached_data
                    self.current_batch_details[cache_key] = {
                        'reward': reward,
                        'rouge': 0.0,
                        'meteor': 0.0,
                        'bert_score': 0.0
        ...
        if need_evaluate_actions:
            ...
            if self.num_gpus <= 1:
                # 单GPU评估
                new_results = self._evaluate_batch(need_evaluate_actions, 0)
            else:
                # 多GPU并行评估
                new_results = []
                ...
                with ProcessPoolExecutor(max_workers=self.num_gpus) as executor:
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            new_results.extend([default_result] * len(batches[len(new_results) % len(batches)]))
            
            # 将新评估的结果添加到缓存和当前批次详细信息
            for i, idx in enumerate(need_evaluate_indices):
                cache_key = cache_keys[idx]
                self.evaluation_cache[cache_key] = new_results[i]
                self.current_batch_details[cache_key] = new_results[i]
        ...
        for cache_key in cache_keys:
            if isinstance(self.evaluation_cache[cache_key], dict):
                rewards.append(self.evaluation_cache[cache_key]['reward'])
            else:
    ...
    def run_optimization(self, num_epochs: int = 20, batch_size: int = 20):
        ...
        for epoch in range(num_epochs):
            ...
            if hasattr(self, 'current_batch_details') and max_reward_key in self.current_batch_details:
                details = self.current_batch_details[max_reward_key]
...
if __name__ == "__main__":
    ...
    project_dir = "../experiments/206-qa20"
