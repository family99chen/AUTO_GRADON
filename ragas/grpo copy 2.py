...
class DirectPolicyNetwork(nn.Module):
    ...
    def forward(self, state=None):
        if state is None:
            # 🔧 使用固定的全0张量替代随机数，确保完全一致的输入
            state = torch.zeros(1, self.num_process, 10)
            print("🎯 使用固定的全0张量作为输入状态，确保完全一致性")
...
class GRPOTrainer:
    ...
    def generate_actions(self, num_samples, epsilon):
        ...
        with torch.no_grad():
            ...
            print(f"[DEBUG GRPO] probs形状: {probs.shape}")
            print(f"[DEBUG GRPO] num_samples: {num_samples}, process_num: {self.process_num}")
            ...
            # 🔧 添加调试信息
            print(f"[DEBUG GRPO] 生成的actions形状: {actions.shape}")
            print(f"[DEBUG GRPO] 生成的actions内容: {actions}")
