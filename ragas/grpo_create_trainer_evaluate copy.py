...
# 🔧 在类外面定义初始化函数
def init_gpu_worker(gpu_id):
    """进程初始化函数，设置专用GPU"""
    import os
    import torch
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    if torch.cuda.is_available():
        torch.cuda.set_device(0)  # 在CUDA_VISIBLE_DEVICES限制下，只有设备0可见
    print(f"🔧 Worker进程初始化完成，使用GPU {gpu_id}")
...
class RAGOptimizer:
    ...
    def setup_grpo_trainer(self, d_model: int = 256, nhead: int = 8, num_layers: int = 3):
        ...
        # 🔧 只需要传入qa_data_path，让trainer内部处理特征提取
        self.trainer = GRPOTrainer(
            ...
            kl_coeff=0.01,
            clip_eps=0.2,
            qa_data_path=self.qa_data_path  # 只传数据路径
        )
    ...
    def _evaluate_single_config(self, actions, gpu_id: int) -> float:
        """单个配置的评估（修复GPU内存分配）"""
        # 🔧 在进程最开始就设置GPU
        import os
        os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
        ...
        import torch
        if torch.cuda.is_available():
            torch.cuda.set_device(0)  # 在单GPU环境下使用设备0
            print(f"🔧 进程使用GPU {gpu_id}，PyTorch设备: {torch.cuda.current_device()}")
        ...
        try:
            ...
            # 🔧 重新初始化评估器，避免使用共享的评估器
            from evaluator import TestEvaluator
            local_evaluator = TestEvaluator(self.qa_data_path, self.corpus_data_path, self.project_dir)
            local_evaluator.init_trial(f"gpu_{gpu_id}_eval")
            
            # 初始化runner
            local_evaluator.init_runner_from_yaml(config_file)
            
            # 评估配置
            yaml_name = f"config_{'_'.join([str(a) for a in actions])}.yaml"
            summary_df = local_evaluator.run_with_qa_eval(yaml_name=yaml_name)
            ...
            self.logger.error(f"[GPU {gpu_id}] 评估出错: {str(e)}")
            return 0.0
    ...
    def _evaluate_batch(self, actions_batch, gpu_id: int) -> List[float]:
        ...
        for i, actions in enumerate(actions_batch):
            ...
            reward = self._evaluate_single_config(actions, gpu_id)
            rewards.append(reward)
            
            eval_time = time.time() - start_time
            self.logger.info(f"[GPU {gpu_id}] 配置 {i+1} 评估完成，奖励={reward:.4f}，耗时={eval_time:.2f}秒")
    ...
    def evaluate_configs(self, actions_batch) -> List[float]:
        ...
        if need_evaluate_actions:
            ...
            if self.num_gpus <= 1:
                ...
                # 🔧 准备参数：(batch, gpu_id, 其他需要的参数)
                task_args = []
                for i, batch in enumerate(batches):
                    gpu_id = i % self.num_gpus
                    self.logger.info(f"🔧 准备批次到GPU {gpu_id}，批次大小: {len(batch)}")
                    # 传递所有需要的参数
                    task_args.append((
                        batch, 
                        gpu_id, 
                ...
                with ProcessPoolExecutor(max_workers=self.num_gpus) as executor:
                    ...
                    for i, args in enumerate(task_args):
                        futures.append(executor.submit(evaluate_batch_with_gpu, *args))
                    ...
                    for i, future in enumerate(as_completed(futures)):
                        try:
                            ...
                            self.logger.info(f"🔧 GPU批次 {i} 完成，获得 {len(batch_rewards)} 个奖励")
                        except Exception as e:
                            self.logger.error(f"🔧 GPU批次 {i} 失败: {str(e)}")
                            # 为失败的批次添加0奖励
                            failed_batch_size = len(batches[i]) if i < len(batches) else 1
                            new_rewards.extend([0.0] * failed_batch_size)
            
            # 将新评估的结果添加到缓存
            for i, idx in enumerate(need_evaluate_indices):
                if i < len(new_rewards):
...
# 🔧 在类外面定义评估函数，避免pickle问题
def evaluate_batch_with_gpu(actions_batch, gpu_id, qa_data_path, corpus_data_path, project_dir, 
                           components, nodes_config, fixed_components):
    """独立的GPU评估函数"""
    import os
    import torch
    ...
    if torch.cuda.is_available():
        ...
        print(f"🔧 Worker进程使用GPU {gpu_id}，PyTorch设备: {torch.cuda.current_device()}")
    ...
    for i, actions in enumerate(actions_batch):
        try:
            # 这里需要实现单个配置的评估逻辑
            # 因为不能直接访问类的方法，需要重新实现或者重构
            
            # 临时简化版本，你可能需要根据实际情况调整
            from evaluator import TestEvaluator
            ...
            for j, action in enumerate(actions):
                if j < len(components):
                    ...
                    node_key = f"node{j+1}"
                    action_idx = str(int(action))
                    if action_idx in nodes_config[node_key]:
                        selected_methods[component] = nodes_config[node_key][action_idx]
            
            for component, method in fixed_components.items():
                selected_methods[component] = method
            
            print(f"🔧 [GPU {gpu_id}] 配置 {i+1}: {selected_methods}")
            ...
            print(f"🔧 [GPU {gpu_id}] 配置 {i+1} 评估失败: {str(e)}")
            rewards.append(0.0)
...
if __name__ == "__main__":
    ...
    project_dir = "../experiments/210-question_features_2"
