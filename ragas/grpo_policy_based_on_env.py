...
# 新增：简单的特征提取函数
def extract_qa_features(qa_data_path: str, num_process: int = 10) -> torch.Tensor:
    """从问答数据提取特征向量"""
    try:
        # 读取数据
        ...
        # 修改：适应你的数据格式，列名是'query'而不是'question'
        if 'query' in df.columns:
            questions = df['query'].head(50).tolist()  # 取前50个问题
        elif 'question' in df.columns:
            questions = df['question'].head(50).tolist()
        else:
            print(f"❌ 找不到问题列，可用列: {df.columns.tolist()}")
            # 失败时返回固定随机特征
        ...
        print(f"🔍 编码后形状: {embeddings.shape}")
        
        # 计算平均特征
        mean_features = embeddings.mean(dim=0)  # [384]
        
        # 调整到10维
        if mean_features.size(0) >= 10:
            features_10d = mean_features[:10]
        else:
            features_10d = torch.cat([mean_features, torch.zeros(10 - mean_features.size(0))])
        
        print(f"🎯 特征向量形状: {features_10d.shape}")
        
        # 扩展到 [1, num_process, 10]
        result = features_10d.unsqueeze(0).unsqueeze(0).expand(1, num_process, -1)
        print(f"✅ 最终特征形状: {result.shape}")
...
class GRPOTrainer:
    def __init__(self, num_process=10, d_model=128, nhead=4, num_layers=3, operation=4, normalize_advantages=True, 
                kl_coeff=0.01, clip_eps=0.2, adv_lambda=0.95, qa_features=None):
        # 初始化当前策略和旧策略网络
        ...
        if qa_features is not None:
            ...
            self.qa_features = qa_features.to(device)  # 移动到与网络相同的设备
            print(f"✅ QA特征已移动到设备: {device}")
        else:
            self.qa_features = None

    def generate_actions(self, num_samples, epsilon):
        """使用旧策略生成动作样本（带Epsilon-Greedy探索）"""
        with torch.no_grad():
            # 修改：使用问答特征（如果有的话）
            logits = self.policy(self.qa_features)  # [1,10,4]
    ...
    def update_policy(self, actions_batch, rewards_batch):
        ...
        with torch.no_grad():
            # 修改：使用问答特征
            old_logits = self.old_policy(self.qa_features)  # 使用旧策略网络
        new_logits = self.policy(self.qa_features)  # 当前策略网络
